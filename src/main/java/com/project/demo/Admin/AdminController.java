...
public class AdminController {
	...
	public ModelAndView adminhome() {
		...
		Integer size=recfeedbackservice.getAllRecFeedback().size();
		...
		Integer overall;
	    if(size != 0)
		  skval=(recfeedbackservice.sksum()*100)/(size*5);
	    else 
	    	skval = 0;
		...
		mv.addObject("skval",skval);
		 ...
		 cpval=(recfeedbackservice.cpsum()*100)/(size*5);
		
		mv.addObject("cpval",cpval);
		if(size == 0)
			apval = 0;
		else
		 apval=(recfeedbackservice.apsum()*100)/(size*5);
		mv.addObject("apval",apval);
		
		if(size == 0)
			overall = 0;
		else
		 overall=(recfeedbackservice.overallsum()*100)/(size*5);
